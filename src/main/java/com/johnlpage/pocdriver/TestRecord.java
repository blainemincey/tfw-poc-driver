...
public class TestRecord {
	...
	TestRecord(int nFields, int depth, int stringLength, int workerID, int sequence, long numberSize, int[] array, int binsize) {
		...
		// Additional fields
		internalDoc.append("claimType", this.getRandomClaimType());
		internalDoc.append("dateClaimSubmitted", this.getRandomDateSubmitted());
		internalDoc.append("claimAmount", this.getRandomBigDecimal());
	...
	private BigDecimal getRandomBigDecimal() {
		BigDecimal bd = new BigDecimal(Double.toString(Math.random() * 100000));
	...
	private String getRandomClaimType() {
		int randomType = rng.nextInt(TestRecord.claimTypes.size());

		return TestRecord.claimTypes.get(randomType);
	}
	 ...
	 * @return
	 */
	private java.util.Date getRandomDateSubmitted() {
